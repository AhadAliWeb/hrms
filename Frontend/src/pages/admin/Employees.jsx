...
import { getEmployees, createEmployee, updateEmployee, deleteEmployee, changeDesignation } from '../../api/employee'
...
export default function Employees() {
  ...
  const [modal, setModal] = useState(null) // null | 'add' | 'edit' | 'designation'
...
}); setSelected(emp); setModal('edit') }
  const openDesig = (emp) => { setSelected(emp); setNewDesignationId(''); setModal('designation') }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else await updateEmployee(selected.id, payload)
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteEmployee(id); toast.success('Employee removed'); load() }
    catch { toast.error('Failed to delete') }
  }

  const handleChangeDesig = async () => {
    if (!newDesignationId) return toast.error('Select a designation')
    setSaving(true)
    try { await changeDesignation(selected.id, newDesignationId); toast.success('Designation changed'); setModal(null); load() }
    catch { toast.error('Failed to change designation') }
    finally { setSaving(false) }
  }

  const filteredDesig = designations.filter(d => {
  console.log('designation departmentId:', d.departmentId, typeof d.departmentId)
  console.log('selected departmentId:', form.departmentId, typeof form.departmentId)

  return !form.departmentId || d.departmentId === Number(form.departmentId)
})

  const columns = [
    { key: 'name', label: 'Name', render: r => `${r.firstName} ${r.lastName}` },
    { key: 'email', label: 'Email' },
    { key: 'departmentName', label: 'Department', render: r => r.departmentName || departments.find(d => d.id === r.departmentId)?.name || '—' },
    { key: 'designationTitle', label: 'Designation', render: r => r.designationTitle || designations.find(d => d.id === r.designationId)?.title || '—' },
    ...
    { key: 'actions', label: 'Actions', render: r => (
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => openDesig(r)} className="p-1.5 hover:bg-violet-50 rounded-lg text-violet-500 transition-colors"><ArrowLeftRight size={14} /></button>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add / Edit Modal */}
      <Modal open={modal === 'add' || modal === 'edit'} onClose={() => setModal(null)} title={modal === 'add' ? 'Add Employee' : 'Edit Employee'} width="max-w-2xl">
      ...
      <Modal open={modal === 'designation'} onClose={() => setModal(null)} title="Change Designation">
        <p className="text-sm text-gray-500 mb-4">Employee: <span className="font-semibold text-gray-800">{selected?.firstName} {selected?.lastName}</span></p>
        <div>
          <label className="block text-xs font-semibold text-gray-600 mb-1">New Designation</label>
          <select value={newDesignationId} onChange={e => setNewDesignationId(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 bg-white">
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button onClick={() => setModal(null)} className="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
          <button onClick={handleChangeDesig} disabled={saving} className="px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors flex items-center gap-2 disabled:opacity-60">
            {saving && <Spinner size="sm" />} Change
          </button>
        </div>
      </Modal>
    </div>
